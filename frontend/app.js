...
function displayIngredientAnalysis(groups, counts) {
    ...
    // Enable the drop-arrow toggle by marking each row expanded/collapsed.
    // When expanded, show an inline paragraph (generated by analysisRow).
    const rows = container.querySelectorAll('.ingredient-analysis-row');

    rows.forEach((row, idx) => {
        const detailsId = `ingredient-details-${Date.now()}-${idx}`;
        row.setAttribute('data-details-id', detailsId);
        row.setAttribute('aria-expanded', 'false');

        // analysisRow() does not include details DOM, so we create one and insert after the row.
        ...
        // Expand with more user-friendly info in simplified English.
        // Row structure: [icon][strong label][span text]
        const type = row.classList.contains('ingredient-analysis-row--safe')
            ? 'safe'
            : row.classList.contains('ingredient-analysis-row--moderate')
                ? 'moderate'
                : 'limit';

        const ingredientsText = row.querySelector('span:nth-of-type(3)')?.textContent || '';
        const simplifiedExplain = ingredientSimplifyHint(type, safe, moderate, limit);

        details.textContent = `${ingredientsText}${simplifiedExplain}`;
...
// Adds short, simplified-English context for the ingredient chips.
function ingredientSimplifyHint(type, safe, moderate, limit) {
    // Prefer actual list sizes over passed arrays when possible.
    const safeCount = Array.isArray(safe) ? safe.length : 0;
    const moderateCount = Array.isArray(moderate) ? moderate.length : 0;
    const limitCount = Array.isArray(limit) ? limit.length : 0;

    if (type === 'safe') {
        if (safeCount) {
            return `\n\n✅ Mostly safe ingredients. These are generally ok for everyday consumption.`;
        }
        return `\n\n✅ Generally fine ingredients.`;
    }

    if (type === 'moderate') {
        if (moderateCount) {
            return `\n\n⚠️ Some ingredients may be moderate (e.g., added sugar/salt or common allergen/carrying ingredients). Best in balance.`;
        }
        return `\n\n⚠️ Moderate ingredients: consume occasionally and check label.`;
    }

    // limit
    if (limitCount) {
        return `\n\n⛔ Ingredients to limit for better health. Consider choosing alternatives with fewer “limit” ingredients.`;
    }
...
function alternativeCard(alt, index) {
    ...
    // Backend images can be protocol-relative: //images.openfoodfacts.org/...
    const normalizedImage = normalizeImageUrl(alt?.image);
    const image = normalizedImage ? normalizedImage : fallbackImage;
    ...
    try {
        ...
        container.innerHTML = data.history.map(item => `
            <div class="history-item">
                <div class="history-name">${escapeHtml(item.name || 'Unknown Product')}</div>
                <div class="history-meta">
                    ${item.category ? `<span class="history-category">${escapeHtml(item.category)}</span>` : ''}
                    ${item.nutrition_grade ? `<span class="history-grade">Grade ${escapeHtml(String(item.nutrition_grade).toUpperCase())}</span>` : ''}
                </div>
